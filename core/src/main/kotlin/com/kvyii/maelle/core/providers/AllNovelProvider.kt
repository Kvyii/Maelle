...
open class AllNovelProvider(client: HttpClient) : MainAPI(client) {
    ...
    override suspend fun loadMainPage(
        ...
        val document = client.get(url, headers = if(requireMobilUserAgent) movilUserAgent else pcUserAgent).document
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val document =
            if(requireMobilUserAgent)
                 client.get("$mainUrl/search?keyword=$query", headers =  movilUserAgent).document
             else
                client.get("$mainUrl/search?keyword=$query").document// AJAX, MIGHT ADD QUICK SEARCH
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val chapterData = client.get(ajaxUrl).document
