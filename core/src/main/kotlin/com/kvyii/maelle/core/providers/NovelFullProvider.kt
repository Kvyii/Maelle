...
class NovelFullProvider(client: HttpClient) : AllNovelProvider(client) {
...
/*
class NovelFullProvider(client: HttpClient) : MainAPI(client) {
    override val name = "NovelFull"
    override val mainUrl = "https://novelfull.com"
    override val hasMainPage = true
    ...
    override suspend fun loadMainPage(
        ...
        val firstresponse = client.get(mainUrl)
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val response =
            client.get("$mainUrl/search?keyword=$query") // AJAX, MIGHT ADD QUICK SEARCH
        ...
        for (h in headers) {
            ...
            val posterUrl =
                mainUrl + Jsoup.parse(client.get(cUrl).text).select("div.book > img").attr("src")
            /*
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val chaptersData =
            client.get("https://allnovel.org/ajax-chapter-option?novelId=$datanovelid")
