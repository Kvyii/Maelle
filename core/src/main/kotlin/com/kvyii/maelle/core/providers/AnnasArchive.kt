...
import com.kvyii.maelle.core.DownloadExtractLink
import com.kvyii.maelle.core.DownloadLink
import com.kvyii.maelle.core.DownloadLinkType
...
import com.kvyii.maelle.core.newEpubResponse
import com.kvyii.maelle.core.newSearchResponse
import com.kvyii.maelle.core.parsed
import org.jsoup.Jsoup

class AnnasArchive(client: HttpClient) : MainAPI(client) {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val text = client.get(url).text.replace(
            Regex("<!--([\\W\\w]*?)-->")
        ) { it.groupValues[1] }
    ...
    private suspend fun loadFromJsonUrl(md5: String): LoadResponse {
        return client.get("$mainUrl/db/md5/$md5.json").parsed<AnnasArchiveRoot>().let { root ->
            newEpubResponse(
